...
export function loadMoreMoviesSuccess(movies) {
  return { type: types.LOAD_MORE_MOVIES_SUCCESS, movies };
}
...
export const loadQueriedMovies = (query, page) => {
  return dispatch => {
    return movieAPI
      ...
      .then(movies => {
        dispatch(loadQueriedMoviesSuccess(movies));
      })
      .catch(error => {
        throw error;
      });
...
export const loadMoreSearchedMovies = (query, page) => {
  return dispatch => {
    return movieAPI
      .searchMovies(query, page)
      .then(movies => {
